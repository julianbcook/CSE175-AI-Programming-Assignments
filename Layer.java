...
public class Layer {
	...
	public void computeOutputDelta() {
		...
		Vector error = targ.difference(act);//This is a Java translation of (Ti - yi): expected - actual.
    	Vector gPrime = net.derivative(min, max);//A Java translation of taking the derivative of (i * ni)

    	for(int i = 0; i < n; i++)
    	{
    		delta.set(i,error.get(i) * gPrime.get(i)); //Sets the computed delta for output units by multiplying error and g'
    	}
	...
	public void computeHiddenDelta() {
		...
		// PLACE YOUR CODE HERE ...
		Vector error = new Vector(n,0.0);//Allocates memory space for an empty Vector

    	for(Projection p : outputs)//Cycles through all of the output projections
    	{
    		Vector temp = p.W.transpose().product(p.output.delta);
    		//The matrix must be transposed in order to maintain equality, as matrix multiplication is not commutative
    		error = error.sum(temp);//Undertakes the Sigma notation of summing all of the matrix multiplications.
    	}

    	Vector gPrime = net.derivative(min, max); //Same function as in the output delta equation

    	for(int i = 0; i < n; i++)
    	{
    		delta.set(i, error.get(i) * gPrime.get(i));//Same as in the output delta equation
    	}
